...
class PopOutView extends View.Abstract {
  ...
  build() {
    ...
    this.element('wrapper', 'div')
      ...
      .style((set) => {
        this.fullScreen = set ? true : this.fullScreen;
      ...
      .style((set) => {
        this.fullScreen = set ? false : this.fullScreen;
  ...
  addHandlers() {
    this.bindListener('resize', window, this.debounce, this.rerender, 250);
  }

  removeHandlers() {
    this.unbindListener('resize', window, this.debounce);
  }
